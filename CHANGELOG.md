...
- Для типа `film` LLM теперь дозаполняет `Director` и `Year` из своих знаний
  о фильме, если в пользовательском тексте названо только название (например
  «посмотрел Интерстеллар, 9 из 10» → `Director=Christopher Nolan`,
  `Year=2014`). Пустыми поля остаются только когда фильм по тексту не
  идентифицируется надёжно.
- Превью заметки теперь дополнительно показывает значение `Rating` в строке
  метаданных, если LLM/пользователь его проставил
  (`<i>🎥 Фильм · 📁 Личное · ⭐⭐⭐⭐⭐⭐⭐⭐⭐</i>`). Работает для всех типов
  с `Rating` property (film, book).
