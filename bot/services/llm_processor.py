...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "- type=film: если в тексте названо только название фильма (без режиссёра "
        "и/или года), но фильм узнаваем по названию — дозаполни `properties.Director` "
        "и `properties.Year` из своих знаний о кино. Например: «посмотрел Интерстеллар, "
        "9 из 10» → Director=\"Christopher Nolan\", Year=\"2014\". Оставляй пустым только "
        "если по тексту фильм не получается надёжно идентифицировать (общее название, "
        "несколько одноимённых фильмов и т.п.). Rating заполняй только если оценка "
        "явно проставлена пользователем — не выдумывай.\n\n"
